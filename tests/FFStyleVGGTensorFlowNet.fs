module FFStyleVGGTensorFlowNet
...
let model(input_img : Tensor, weights : Map<string,Tensor>) =
    ...
    let instance_norm(input:Tensor, train:bool, name:string) =
        ...
        //let normalized = tf.div((graph.Sub(input,mu),graph.Pow(graph.Add(sigma_sq,epsilon),graph.Const(new TFTensor(0.5f))))
        let normalized = failwith "todo"
