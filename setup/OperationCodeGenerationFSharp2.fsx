// This is to wrap TensorFlow.Net dynamic generation

// NOTE: TFTensor -> Tensor
...
let run(dirs : string []) =
    ...
    let fillArguments (oper : OpDef, requiredAttrs : OpDef.AttrDef[], optionalAttrs : OpDef.AttrDef[]) =
        [|
            yield! oper.InputArgs |> Seq.toArray |> Array.map (fun inarg -> sprintf "%s : %s" (paramMap inarg.Name) (if isListArg inarg then "Tensor[]" else "Tensor") ) 
...
//    let makeRecordType (depth : int, oper : OpDef, requiredAttrs : OpDef.AttrDef[], optionalAttrs : OpDef.AttrDef[]) =
    ...
    let generate (oper : OpDef) =
        ...
        let retType = 
            if hasReturnValue then
                ...
                | xs -> xs |> Array.map (fun arg ->  (if isListArg arg then "Tensor[]" else "Tensor")) |> String.concat "*" |> sprintf "(%s)"
            else "Operation"
        ...
        p "let dict = new Dictionary<string,obj>()"
        ...
        if requiredAttrs.Length > 0 || optionalAttrs.Length > 0 then
            ...
            for attr in optionalAttrs do
                ...
                p (sprintf "%s |> Option.iter (fun x -> dict.[\"%s\"] <- x)" (paramMap attr.Name) attr.Name)
        
        p (sprintf "let name = defaultArg name \"%s\"" name)
        p (sprintf "let op = _op_def_lib.Force()._apply_op_helper(\"%s\", name = name,keywords = dict)" name )
...
//            p "let mutable _n = 0"
        for arg in oper.OutputArgs do
            if isListArg arg then
                ...
                p (sprintf "let %s = [| for i = 0 to %s - 1 do yield op.outputs.[_idx]; _idx <- _idx + 1; |]" (paramMap arg.Name) n)
            else
                p (sprintf "let %s = op.outputs.[_idx]" (paramMap arg.Name))
                p "_idx <- _idx + 1"
        if hasReturnValue then
//            if oper.OutputArgs.Count = 1 then
//                p (paramMap oper.OutputArgs
//            else
            p (sprintf "(%s)" (oper.OutputArgs |> Seq.map (fun  x -> paramMap x.Name ) |> String.concat ", "))
        else p "op"
        pd "\n"
            
    p "namespace Tensorflow"
    p "open System"
    p "open System.Collections.Generic\n"

    // TODO generate Args types


    pi "type GenOps() ="
    p "static let _op_def_lib = lazy new OpDefLibrary()"
...
|> fun res -> File.WriteAllText(Path.Combine(root,"src", "TensorFlow.FSharp", "TFNetWrapper.g.fs"), res)
